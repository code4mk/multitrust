...
trait HasRoleAndPermission
{
    ...
    {
        $permissionModel = app(config('multitrust.models.permission'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[multitrust.models.permission] must be an instance of \Illuminate\Database\Eloquent\Model');
        }
    ...
    {
        return (bool) config('multitrust.pretend.enabled');
    }
    ...
    {
        return (bool) config('multitrust.pretend.options.' . $option);
    }
    ...
    public function callMagic($method, $parameters)
    {
        if (starts_with($method, 'is')) {
            return $this->hasRole(snake_case(substr($method, 2), config('multitrust.separator')));
        } elseif (starts_with($method, 'can')) {
            return $this->hasPermission(snake_case(substr($method, 3), config('multitrust.separator')));
        } elseif (starts_with($method, 'allowed')) {
            return $this->allowed(snake_case(substr($method, 7), config('multitrust.separator')), $parameters[0], (isset($parameters[1])) ? $parameters[1] : true, (isset($parameters[2])) ? $parameters[2] : 'user_id');
        }
